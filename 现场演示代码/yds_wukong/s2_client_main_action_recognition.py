...
if __name__ == "__main__":
    ...
    if device:
        ...
        #-----------------------------------------
        #images_loader = select_images_loader(SRC_DATA_TYPE, SRC_DATA_PATH) ################调用其他平台的摄像头，一定要注释掉
            ...
            while True :    #images_loader.has_image():
                ...
                if len(dict_id2skeleton):
                    ...
                    if start_flag:
                        ...
                        #
                        #             flag_stand = False
                        if dict_id2label[min_id] == 'kick' and flag_kick:
                            if last_action.count('kick') ==5:

                                    asyncio.get_event_loop().run_until_complete(test_play_tts10())
                                    sock_tonypi.send(b'kick')
                                    ###挥手
                        ...
                        elif dict_id2label[min_id] == 'walk' and flag_walk:
                            if last_action.count('walk') == 5:
                                asyncio.get_event_loop().run_until_complete(test_play_tts5())
                                sock_tonypi.send(b'walk')
                                ###挥手
                                asyncio.get_event_loop().run_until_complete(test_play_action2())

                                asyncio.get_event_loop().run_until_complete(test_play_tts5_1())
                                flag_walk = False
                        elif dict_id2label[min_id] == 'wave' and flag_wave:
                            if last_action.count('wave') == 5:
                                    asyncio.get_event_loop().run_until_complete(test_play_tts6())
                                    sock_tonypi.send(b'wave')
                                    asyncio.get_event_loop().run_until_complete(test_play_action3())

                                    flag_wave = False
                        elif dict_id2label[min_id] == 'punch' and flag_punch:
                            if last_action.count('punch') == 5:
                                asyncio.get_event_loop().run_until_complete(test_play_tts8())
                                sock_tonypi.send(b'punch')
                                ###挥手
                                asyncio.get_event_loop().run_until_complete(test_play_action4())

                                asyncio.get_event_loop().run_until_complete(test_play_tts5_1())
                                flag_punch = False

                        elif dict_id2label[min_id] == 'squat' and flag_squat:
                            if last_action.count('squat') == 5:
                                asyncio.get_event_loop().run_until_complete(test_play_tts9())
                                sock_tonypi.send(b'squat')
                                ###挥手
                                asyncio.get_event_loop().run_until_complete(test_play_action5())
                                asyncio.get_event_loop().run_until_complete(test_play_action6())
                                asyncio.get_event_loop().run_until_complete(test_play_tts5_1())
                                flag_squat = False

                        # elif dict_id2label[min_id] == 'sit' and flag_sit:
                        #     if last_action.count('sit') == 5:
                        #
                        #             asyncio.get_event_loop().run_until_complete(test_play_tts6())
