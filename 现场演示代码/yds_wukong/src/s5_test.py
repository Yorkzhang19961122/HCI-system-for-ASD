...
import cv2
import argparse
if True:  # Include project path 包括项目路径
    import sys
    import os
    ...
    from utils.lib_emotion_classifier import Emotion_classifier

def par(path):  # Pre-Append ROOT to the path if it's not absolute 如果不是绝对路径，则将root预先添加到路径
...
def get_dst_folder_name(src_data_type, src_data_path):
    ''' Compute a output folder name based on data_type and data_path. 根据data_type和data_path得到输出文件夹名称
...
# emotion_classifier_setting
EMO_MODEL_FOLDER = cfg["emotion_classifier"]["model_folder"]
...
# If data_type is video, set the sampling interval. 设置视频的采样间隙
# For example, if it's 3, then the video will be read 3 times faster. 如果为3，则视频读取速度将为3倍
...
def select_images_loader(src_data_type, src_data_path):
    if src_data_type == "video":
        ...
        images_loader = lib_images_io.ReadFromWebcam(
            SRC_WEBCAM_MAX_FPS, webcam_idx)  # 实例化ReadFromWebcam类
    return images_loader  # 返回的就是实例化以后的ReadFromVideo/Folder/Webcam类


class MultiPersonClassifier(object):
    ''' This is a wrapper(包装) around ClassifierOnlineTest
...
def draw_result_img(img_disp, ith_img, humans, dict_id2skeleton,
    ...
    # Resize to a proper size for display 调整到适合显示的大小
    ...
    # Draw all people's skeleton 绘制所有人的骨架
    skeleton_detector.draw(img_disp, humans)

    # Draw bounding box and label of each person 绘制每个人的边框和标签
    ...
    # Add blank to the left for displaying prediction scores of each class 在左侧增加一块空白用于显示各类的分数
    img_disp = lib_plot.add_white_region_to_left_of_image(img_disp)

    cv2.putText(img_disp, "Frame:" + str(ith_img),
                (20, 20), fontScale=1.0, fontFace=cv2.FONT_HERSHEY_PLAIN,
                color=(0, 0, 0), thickness=1)

    # Draw predicting score for only 1 person 显示一个人的预测分数
    if len(dict_id2skeleton):
...
if __name__ == "__main__":
    ...
    # -- Detector, tracker, classifier 实例化检测器、跟踪器、分类器

    skeleton_detector = SkeletonDetector(OPENPOSE_MODEL, OPENPOSE_IMG_SIZE)

    multiperson_tracker = Tracker()
    # 将MultiPersonClassifier类实例化，创建实例multiperson_classifier
    multiperson_classifier = MultiPersonClassifier(SRC_MODEL_PATH, CLASSES)
    # 实例化emotion_classifier
    # emotion_classifier = Emotion_classifier(EMO_MODEL_FOLDER)
    # -- Image reader and displayer 图像读取器和显示器
    images_loader = select_images_loader(SRC_DATA_TYPE, SRC_DATA_PATH) # 将实例化以后的ReadFromVideo/webcam/folder类赋给images_loader
    img_displayer = lib_images_io.ImageDisplayer() # 实例化img_displayer

    # -- Init output

    # output folder 创建输出的文件夹
    os.makedirs(DST_FOLDER, exist_ok=True)
    os.makedirs(DST_FOLDER + DST_SKELETON_FOLDER_NAME, exist_ok=True)

    # video writer 实例化video_writer
    video_writer = lib_images_io.VideoWriter(
        DST_FOLDER + DST_VIDEO_NAME, DST_VIDEO_FPS)

    # -- Read images and process 读取图像并处理
    try:
        ith_img = -1
        while images_loader.has_image(): # images_loader就是实例化以后的ReadFromVideo/webcam/folder类

            # -- Read image
            img = images_loader.read_image() # read_image 方法
            # print('image:', img)
            ith_img += 1
            img_disp = img.copy()
            ...
            # -- Draw emotion
            # img_disp = emotion_classifier.emotion_classify(img_disp)
            # -- Draw
            ...
            if len(dict_id2skeleton):
                ...
                print("prediced label is :", dict_id2label[min_id]) # 只显示ID号最小的标签
