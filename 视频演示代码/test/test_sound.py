...
async def test_play_tts4():
    ...
    block: PlayTTS = PlayTTS(text="坐了这么久，是该站起来活动一下了", control_type=TTSControlType.START)
    block1: PlayTTS = PlayTTS(text="甩甩手吧", control_type=TTSControlType.START)
    # 返回元组, response是个ControlTTSResponse
    #(resultType, response) = await block2.execute()
    (resultType, response) = await block.execute()
    (resultType, response) = await block1.execute()
...
async def test_play_tts5():
    # control_type: TTSControlType.START: 播放tts; TTSControlType.STOP: 停止tts
    block: PlayTTS = PlayTTS(text="做的真棒,我们也来做一个", control_type=TTSControlType.START)
    await asyncio.sleep(3)
    # 返回元组, response是个ControlTTSResponse
    (resultType, response) = await block.execute()
...
async def test_play_tts6():
    # control_type: TTSControlType.START: 播放tts; TTSControlType.STOP: 停止tts
    block: PlayTTS = PlayTTS(text="你完成的太好了,我们也来试一下", control_type=TTSControlType.START)
    # 返回元组, response是个ControlTTSResponse
    (resultType, response) = await block.execute()
