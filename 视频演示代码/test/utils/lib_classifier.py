...
class ClassifierOfflineTrain(object):
    ...
    def _predict_proba(self, X):
        ...
        Y_probs = self.clf.predict_proba(self.pca.transform(X)) #概率估计
...
class ClassifierOnlineTest(object):
    ...
    def draw_scores_onto_image(self, img_disp):
        ...
        for i in range(-1, len(self.action_labels)):
            ...
            # print("i:",i)
            # 在屏幕上对做出的正确姿势给出反馈
            # scores2float = ('%.2f' % self.scores[i])
            # print('scores2float:', scores2float)
            if self.scores[i] > self.THRESHOLD_SCORE_FOR_DISP:
                cv2.putText(img_disp, text='Action: {} is correct!'.format(self.action_labels[i]), org=(540, 20),
                            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=FONT_SIZE,
                            color=(0, 0, 255), thickness=2)
                cv2.putText(img_disp, text='Score:{:.1f}'.format((self.scores[i])*100), org=(540, 40),
                            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=FONT_SIZE,
                            color=(0, 0, 255), thickness=2)
            # else:
                # cv2.putText(img_disp, text='Waiting!', org=(350, 50),
                #             fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=FONT_SIZE,
                #             color=(0, 0, 255), thickness=2)

            # print("self.scores_raw:", self.scores)
            # print("self.scores:{:.2f}".format(self.scores[i]))
